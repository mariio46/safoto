<x-auth-layout title="{{ Str::title($user->username) }}">

    <div class="grid grid-cols-1 xl:grid-cols-3 xl:gap-4">
        <div class="mb-4 md:flex justify-between items-center col-span-full xl:mb-2">
            <h1 class="text-xl font-semibold text-gray-900 pt-2">{{ Str::title($user->username) }} profile</h1>
            <div class="flex flex-wrap space-x-2 pt-2">
                <a href="{{ route('usersmanagement.index') }}"
                    class="flex items-center py-2 px-2 rounded-md font-bold bg-red-500 bg-opacity-80 text-white shadow-md sm:hover:scale-110 transition duration-300 ease-in-out">Back</a>
                <a href="{{ route('usersmanagement.edit', $user) }}"
                    class="flex items-center text-sm bg-clip-padding bg-gradient-to-r to-emerald-600 from-sky-400 px-4 py-2 md:px-5 font-bold text-white rounded-md sm:hover:scale-110 transition duration-300 ease-in-out">Update</a>
            </div>
        </div>
        <!-- Right Content -->
            ...
            <div class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 sm:p-6 ">
                <div class="flex items-center justify-center ">
                    <img class="mb-4 rounded-lg w-52 h-auto  sm:mb-0 xl:mb-4 2xl:mb-0"
                        src="{{ asset('../storage/' . $user->profile) }}" alt="{{ $user->username }}">
                </div>
            </div>
            <div class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 sm:p-6 ">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="username" id="username"
                        ...
                        placeholder="Bonnie" readonly disabled required value="{{ $user->username }}">
                </div>
                <div class="mb-4">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Email</label>
                    <input type="email" name="email" id="email"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 "
                        placeholder="Bonnie" readonly disabled required value="{{ $user->email }}">
                </div>
            </div>
        </div>
        @if ($biodata->count())
        ...
        @else
            <div class="col-span-2">
                <div
                    ...
                    <h3 class="text-lg font-semibold ">This user didn't have biodata</h3>
                    <form action="{{ route('usersmanagement.bioStore') }}" method="post">
                        @method('post')
                        @csrf
