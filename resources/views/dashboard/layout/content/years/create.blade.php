<x-auth-layout title="Upload Year">
        ...
        <div class="p-4 mb-0 space-y-6 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2">
            ...
            <form action="{{ route('years.store') }} " method="POST">
                ...
                <div class="grid gap-6 mb-3 lg:grid-cols-2">
                    <div>
                        <label for="yearName"
                            class="block mb-2 text-sm font-medium text-gray-900  @error('yearName')text-red-500 @enderror">Year's
                            City</label>
                        <input type="text" id="yearName" name="yearName"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 placeholder:italic placeholder:text-[13px] @error('yearName')border-red-500 placeholder:text-red-500 @enderror"
                            placeholder="Kota Makassar" autofocus>
                        <x-validation-error name="yearName" />
                    </div>
                </div>
